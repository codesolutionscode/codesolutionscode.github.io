...
$(window).on("load", function () {
  TweenLite.to($(".loading .txt"), 0.4, { autoAlpha: 1, y: 0 });
  TweenLite.to($(".progress"), 0.5, { autoAlpha: 1, delay: 0.4 });
...
$(document).ready(function () {
  ...
  $("body").on(
    "click",
    '[data-type="ajax-load"], header ul li a, .logo',
    function (e) {
  ...
  function infinty_loop() {
    if ($(".forslide").length) {
      ...
      function mouseEnter() {
        ...
        $(".forslide.slider-images li:eq(" + list_index + ")").addClass(
          "focus"
        );
  ...
  function letter_slider() {
    if ($("#letter-slider").length) {
      ...
      for (let i = 0; i < lnx.length; i++) {
        ...
        $(".bullets ul").append(
          '<li class="magnetize" data-dist="1"> <span data-cursor-type="medium">' +
            txt +
            "\n" +
            "</span></li>"
        );
      ...
      $(
        ".bullets ul li:first-child, .slider-images ul li:first-child"
      ).addClass("focus");
        ...
        pagination: {
          ...
          renderBullet: function (index, className) {
            return (
              ...
              className +
              '">' +
              '<div data-cursor-type="medium" class="bullet-outter">' +
              '<svg data-cursor-type="medium" class="fp-arc-loader" width="20" height="20" viewBox="0 0 20 20">' +
  ...
  function allworks() {
    if ($(".allworks").length) {
      ...
      if (main_works >= 4) {
        ...
        var scrollBottom = $(
          ".allworks .menu__item:eq(" + main_works + ")"
        ).offset().top;
        ...
        $(".allworks").on("scroll", function (event) {
          ...
          var works_height =
            $(".allworks .menu__item").height() * main_works * 2;
    ...
    if ($(".forslide").length) {
      ...
      function mouseEnter() {
        ...
        if (list_index >= total_image) {
          list_index = list_index % total_image;
          console.log("bÃ¼yÃ¼k");
        } else {
          list_index = $(this).index() / 2;
          console.log("deÄŸil");
        }
        ...
        $(".forslide.slider-images li:eq(" + list_index + ")")
          .find("video")
          .each(function () {
            $(this).get(0).play();
          });
  ...
  function PortfolioGrids() {
    if ($(".masonry").length) {
      ...
      $(".filter-icon").on("click", function () {
        TweenMax.to(".portfolio-filter", 0.3, { autoAlpha: 1 });
        TweenMax.staggerFrom(
          ".portfolio-filter ul > li",
          0.5,
          { y: "50", autoAlpha: 0, ease: Back.easeOut },
          0.2
        );
      });
      $(".close-filter,  .portfolio-filter ul li a").on("click", function () {
        TweenMax.to(".portfolio-filter", 0.3, { autoAlpha: 0, delay: 0.5 });
      });
      $(".portfolio-filter, .portfolio-filter ul li a").on(
        "click",
        function (event) {
          if (!$(event.target).is(".portfolio-filter ul li a")) {
            TweenMax.to(".portfolio-filter", 0.3, { autoAlpha: 0 });
            return false;
          }
        }
      );
      ...
      $(window).on("scroll", function () {
        ...
        if (sw >= 200) {
          ...
          TweenMax.from(bottom_items, 0.5, { y: 0, autoAlpha: 1 }, 1);
        } else {
          $(bottom_items).removeClass("magnetize");
          TweenMax.to(bottom_items, 0.5, { y: 200, autoAlpha: 0 }, 1);
        }
      });
      ...
      $(".grid-item .portfolio-item").on("mouseenter", function () {
        TweenMax.to(".portfolio-big-title", 0.5, { y: 100, autoAlpha: 1 }, 1);

        if ($(this).data("title")) {
          $(".portfolio-big-title .title").html(
            "<span>" + $(this).data("title") + "</span>"
          );
        }

        if ($(this).data("category")) {
          $(".portfolio-big-title .category").html(
      ...
      $(".grid-item .portfolio-item").on("mouseleave", function () {
        TweenMax.to(".portfolio-big-title", 0.5, { y: 0, autoAlpha: 0 }, 1);
      });
  ...
  function cursorMoveFunc() {
    ...
    function t(t) {
      ...
      TweenMax.to(e, 0, {
        ...
        "medium" == $(t.target).data("cursor-type")
          ? (e.removeClass().addClass("is-medium"), n())
          : "big" == $(t.target).data("cursor-type")
          ? "btn-play" == $(t.target).data("cursor-text")
            ? (e.removeClass().addClass("is-play").addClass("is-big"), n())
            : "btn-pause" == $(t.target).data("cursor-text")
            ? (e.removeClass().addClass("is-pause").addClass("is-big"), n())
            : (e.removeClass().addClass("is-view").addClass("is-big"),
              e.find(".cursor__label").text($(t.target).data("cursor-text")))
          : (e.removeClass(), n());
    }
...
/*----------------------------------------------------------------------*/
  ...
  function header_options() {
    if ($(".split-menu").length) {
      var full_menu_animation = new TimelineMax({
        yoyo: false,
        reversed: true,
      });
      full_menu_animation.pause();
      full_menu_animation.to("header .hamburger span", 0.1, {
        backgroundColor: "#fff",
        ease: Back.easeOut,
      });
      full_menu_animation.to("header .hamburger span:nth-child(1)", 0.1, {
        rotation: 45,
        ease: Back.easeOut,
      });
      full_menu_animation.fromTo(
        ["header .hamburger span:nth-child(1)"],
        0.1,
        { width: "70%" },
        { width: "100%" }
      );
      full_menu_animation.to("header .hamburger span:nth-child(2)", 0.1, {
        rotation: -45,
        y: 6,
        ease: Back.easeOut,
      });
      full_menu_animation.to(".hamburger", 0.3, {
        width: "20px",
        ease: Back.easeOut,
      });
      full_menu_animation.to(".hamburger", 0.3, {
      ...
      full_menu_animation.from(
        "header .split-menu .split-left-side",
        0.8,
      ...
      full_menu_animation.from(
        "header .split-menu .split-right-side",
        0.8,
        { x: "100%", ease: Power3.easeOut },
        0.2,
        "Start"
      );
      full_menu_animation.staggerFrom(
        "header .full-menu nav ul li:not(header .full-menu nav ul li ul li)",
        0.5,
        { autoAlpha: 0, x: -50 },
        0.2,
        "Start"
      ...
      full_menu_animation.staggerFrom(
        "header .split-menu .split-right-side ul",
        0.5,
        { autoAlpha: 0, x: 50 },
        0.2,
        "Start"
      ...
      $("header .hamburger").on("click", function () {
        full_menu_animation.reversed()
          ? full_menu_animation.play()
          : full_menu_animation.reverse(),
          $("header .full-menu ul li.has-sub ul").slideUp(),
          $("header .full-menu ul li.has-sub i").removeClass("active");
      });

      $("header .full-menu ul li.has-sub ul").slideUp();

      $("header .full-menu ul li.has-sub i").on("click", function () {
        $("header .full-menu ul li.has-sub ul")
      ...
      full_menu_animation.from(
        "header .full-menu:not('header .full-menu.right-menu')",
        0.8,
    ...
    $("header .full-menu nav ul li.has-sub").append(
      '<i class="fas fa-arrow-right" data-cursor-type="medium"></i>'
    );
  ...
  function map() {
    if ($(".map").length) {
      $(".open-map").on("click", function () {
        TweenMax.to(".map", 0.3, { autoAlpha: 1, display: "block" });
        TweenMax.to(".overlay-map", 0.2, { display: "block", autoAlpha: 1 });
      });

      $(".overlay-map").on("click", function () {
        TweenMax.to(".map", 0.3, { autoAlpha: 0, display: "none" });
...
/*----------------------------------------------------------------------*/
  ...
  function others() {
    if ($("header.classic-menu").length) {
      $(
        "header.classic-menu nav ul li a:not('header.classic-menu nav ul li ul li a')"
      ).hover(
        function () {
          $("header.classic-menu nav ul li a")
            .not(this)
            .not("header.classic-menu nav ul li ul li a")
            .css("opacity", ".5");
        },
        function () {
          $("header.classic-menu nav ul li a")
    ...
    if ($(window).width() <= 991) {
      ...
      sub_menu.pause();
      sub_menu.from($("header.classic-menu nav ul li").find("ul"), 0.2, {
        height: "0",
        autoAlpha: 0,
        display: "none",
      });
      sub_menu.to($("header.classic-menu nav ul li").find("i"), 0.1, {
        rotation: 180,
      });
      ...
      responsive_menu.to(
        "header.classic-menu .close-menu span:first-child",
        0.2,
        { rotation: 45, y: 6 }
      );
      responsive_menu.to(
        "header.classic-menu .close-menu span:last-child",
        0.2,
        { width: "100%", rotation: -45 }
      );
      responsive_menu.from(
        "header.classic-menu nav",
        0.5,
        { display: "none", autoAlpha: 0, ease: Back.easeOut },
        0.2
      );
      responsive_menu.staggerFrom(
        "header.classic-menu nav ul li:not('header.classic-menu nav ul li ul li')",
        0.6,
        { y: "50", autoAlpha: 0, ease: Back.easeOut },
        0.1
      );
      ...
      $("header.classic-menu .close-menu").on("click", function () {
        responsive_menu.reversed()
          ? responsive_menu.play()
          : responsive_menu.reverse();
      });
    }
